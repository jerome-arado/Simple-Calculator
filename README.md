# Simple Calculator

A clean, dark-themed Simple Calculator built with HTML, CSS, and JavaScript.  
This project demonstrates basic front-end web development, focusing on layout design, interactive functionality, and modern UI styling.

# Features

- Responsive calculator layout with grid-based design
- Dark mode theme for modern aesthetics
- Supports basic arithmetic operations: addition, subtraction, multiplication, division
- Functional C (clear) and DEL (delete last) buttons
- Error handling for invalid expressions
- Animated button interactions and subtle hover effects
- Clean, readable display screen


# Technologies Used
HTML5 - Defines the structure and content of the calculator.
CSS3 - Styles the layout, colors, animations, and grid system.
JavaScript - Handles button functionality and display logic.


# How to Use

1. Clone the repository
   - git clone https://github.com/jerome-arado/Simple-Calculator.git
2. Navigate into then folder
   - cd Simple-Calculator
3. Open the project
   - Simply open index.html in your browser.

# Author 
Jerome Arado
BSIT 3-3
